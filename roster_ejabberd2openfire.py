...
while ejdump.find("{roster", fromidx) != -1:
    ...
    if "record_name" in finstr:
        ...
        pprint.pprint( finstr.replace("{","").replace("}","").replace('"',"").replace("\n","").split(",")[1:3] )
        uid, domain = finstr.replace("{","").replace("}","").replace('"',"").replace("\n","").split(",")[1:3]
        userpws[(uid,domain)] = uid[0:-1]
        fromidx=eidx
...
for user,pw in userpws.iteritems():
  ...
  <User>
    ...
    <Roster>"""%(user[0].replace("\\\\","\\"),"",int(time.time()), int(time.time())))
    ...
    if user in rosters.keys():
      ...
      <Item jid="%s" askstatus="%s" recvstatus="%s" substatus="%s" name="%s" >
        <Group>%s</Group>
                ...
                ofirexml.write("""
      <Item jid="%s" askstatus="%s" recvstatus="%s" substatus="%s" name="%s" />"""%( jid.replace("\\\\","\\"), askst, recvst, subst, i[1]))
